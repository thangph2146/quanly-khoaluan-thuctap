...
import type { Lecturer, CreateLecturerData, UpdateLecturerData } from '../types'
...
export function UnifiedLecturersContainer({
    ...
    const searchParams = useMemo(() => {
        ...
        if ((showDepartmentFilter || showFilterUI) && selectedDepartmentId && selectedDepartmentId !== 'all') {
    ...
    }, [selectedDepartmentId, debouncedSearchTerm, showDepartmentFilter, showFilterUI])

    const { lecturers, isLoading, refetch } = useLecturers(searchParams)
    const { departments, isLoading: isLoadingDepartments } = useDepartments()
    const { createLecturer, updateLecturer, deleteLecturer, isCreating, isUpdating, isDeleting } = useLecturerActions(refetch)
    ...
    const selectedDepartment = useMemo(() => {
        ...
        return departments.find(dept => dept.id === parseInt(selectedDepartmentId))
    ...
    const handleFormSubmit = async (data: CreateLecturerData | UpdateLecturerData) => {
        try {
            // If creating and a department is selected, set it as default
            if (sheetMode === 'create' && selectedDepartmentId && selectedDepartmentId !== 'all' && !data.departmentId) {
        ...
        <div className="space-y-6">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        ...
                                        >
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder={isLoadingDepartments ? "Đang tải..." : "Chọn tất cả hoặc một khoa/đơn vị"} />
                                            </SelectTrigger>
                                            <SelectContent>
            ...
            {showDepartmentInfo && selectedDepartment && (
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Mã đơn vị: {selectedDepartment.code} •
                            Số lượng giảng viên: {filteredLecturers.length}
                        </CardDescription>
                    </CardHeader>
                </Card>
            )}
            ...
            <Sheet open={isSheetOpen} onOpenChange={setSheetOpen}>
                <SheetContent className="sm:max-w-lg">
                    <SheetHeader>
                        <SheetTitle>
                            {sheetMode === 'create' ? 'Tạo giảng viên mới' : 'Chỉnh sửa giảng viên'}
                        </SheetTitle>
                        <SheetDescription>
                            {sheetMode === 'create'
                                ? `Điền thông tin để tạo một giảng viên mới${selectedDepartment ? ` cho ${selectedDepartment.name}` : ''}`
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Bạn có chắc chắn muốn xóa giảng viên "{selectedLecturer?.name}" không?
            ...
            <Dialog open={isDetailsDialogOpen} onOpenChange={setDetailsDialogOpen}>
                <DialogContent className="sm:max-w-2xl">
                    ...
                    {selectedLecturer && (
                        <LecturerDetails lecturer={selectedLecturer} />
                    )}
                </DialogContent>
            </Dialog>
        </div>
