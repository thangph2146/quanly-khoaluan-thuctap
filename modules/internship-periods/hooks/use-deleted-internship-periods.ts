...
export function useDeletedInternshipPeriods(filters: InternshipPeriodFilters) {
  ...
  const fetchInternshipPeriods = useCallback(async (currentFilters: InternshipPeriodFilters) => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await InternshipPeriodService.getDeleted(currentFilters);
      ...
      setError(err instanceof Error ? err.message : 'Lỗi khi tải danh sách đợt thực tập đã xóa');
      setInternshipPeriods([]);
      setTotal(0);
    } finally {
      setIsLoading(false);
  ...
  return {
    deletedInternshipPeriods: internshipPeriods,
    setDeletedInternshipPeriods: setInternshipPeriods,
    total,
    isLoading,
    error,
